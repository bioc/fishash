...
test_that("Fishash results on test dataset haven't changed", {
    ...
    expected_result <- readRDS(test_path(
        'fixtures',
        'fishash_schraivogel.rds'
    ))

    new_result <- fishash(counts(altExp(tapseq_diffex)))
    ...
    expect_equal(
        metadata(expected_result)$log_pval_cutoff,
        metadata(new_result)$log_pval_cutoff
    )

    expect_identical(
        colData(expected_result)$demux_type,
    ...
    expect_identical(
        assay(expected_result, 'assigned'),
