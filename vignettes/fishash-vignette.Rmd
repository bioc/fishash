...
We illustrate fishash on a TAPseq dataset [@schraivogel2020targeted]
dataset which is also bundled with the
...
expression for 72 genes and CRISPRi perturbations consisting of 86
gRNAs.  The data is represented as a `SingleCellExperiment`.
...
Note that TAPseq ("Targeted Perturbseq") amplifies transcripts from a
panel of genes (in this case, 72 genes, mostly on chromosome 8)
instead of sequencing the whole transcriptome.

The gRNA assay is stored in the `altExp()`:
...
of 4 genes. Each gene target has 4 guides targetting its promoter or
...
Next, we call `fishash()` to assign the guides:
...
guides per cell by using the `colSums` of the assigned assay:
...
# Annotate the assigned target gene
colData(singlets)$target_gene <- rowData(altExp(singlets))[
    colData(singlets)$assignment, "target_gene"
]
```

Next, we use `r Biocpkg("glmGamPoi")` to perform negative binomial
regression of the cells from each guide against the NTC cells,
...
res_glmgp <- bplapply(
    unique(colData(singlets)$assignment),
    function(g) {
        keep <- with(colData(singlets), target_gene == "NTC" | assignment == g)
...
res_glmgp <- do.call(rbind, res_glmgp)

# recompute adjusted p-values over all tests
res_glmgp$adj_pval <- p.adjust(res_glmgp$pval, method = "BH")
```

We then plot a heatmap of the shrunken log-fold changes 
(computed with `r CRANpkg("ebnm")`)
overlaid with the significant genes at FDR 5%.

```{r de-heatmap, fig.height=15, fig.width=15, fig.wide=TRUE}
library(ebnm)
library(ComplexHeatmap)

# compute shrunken log fold changes

res_glmgp$lfc_shrunk <- ebnm(
    res_glmgp$lfc,
    res_glmgp$lfc_se,
    prior_family = "ash"
)$posterior$mean

# unmelt long dataframe of results into wide matrices
res_glmgp %>%
    select(name, grna, lfc_shrunk) %>%
    pivot_wider(names_from = "grna", values_from = "lfc_shrunk") %>%
...
res_glmgp %>%
    select(name, grna, adj_pval) %>%
    pivot_wider(names_from = "grna", values_from = "adj_pval") %>%
    column_to_rownames("name") %>%
    as.matrix() %>%
    .[rownames(mat_lfc_shrunk), colnames(mat_lfc_shrunk)] ->
mat_padj

# annotation of significant p-values
mat_star <- matrix("",
    nrow = nrow(mat_padj), ncol = ncol(mat_padj),
    dimnames = dimnames(mat_padj)
)
mat_star[mat_padj <= .05] <- "x"

Heatmap(
    t(mat_lfc_shrunk),
    right_annotation = rowAnnotation(
        df = rowData(altExp(tapseq_diffex))[
            colnames(mat_lfc_shrunk),
            "target_element_type",
            drop = FALSE
        ],
    ...
    layer_fun = function(j, i, x, y, w, h, col) {
        grid.text(
            t(mat_star)[cbind(i, j)], x, y,
            gp = gpar(fontsize = 12, fontface = "bold")
        )
    },
    cluster_rows = FALSE, cluster_columns = FALSE,
    heatmap_legend_param = list(title = "shrunkLogFC")
)
```
...
particular with the target gene usually being knocked down (e.g. CPQ guides
have negative logFC for CPQ gene expression).  Also, the NTC guides usually
have no DEGs, with some exceptions (such as NTC-22) which may be worth
investigating as outliers. There may also be other outlier guides to
exclude -- for example, the fourth PHF20L1 guide has different DEGs
from the other PHF20L1 guides and in particular fails to knockdown the
PHF20L1 gene.

The differential expression results could possibly be improved with QC
filtering of cells, controlling for confounders (such as cell state),
better normalization, better dispersion estimates, or more generally
investigating sources of model misspecification -- a more detailed
analysis is beyond the scope of this vignette.
