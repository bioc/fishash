...
simulate_guidebender <- function(
    ...
    guide_infection_freqs <- as.vector(rdirichlet(
        1,
        guide_infection_alpha * rep(1, n_guides)
    ))
    ...
    chi_g_a <- (1 - endo_shape_flat) *
        chi_g_a +
        endo_shape_flat * rep(endo_shape_sum, n_guides)
...
simulate_guidebender_helper <- function(
    ...
    lambd_ng_noise_exo <- sweep(
        rowSums(lambd_ng_endo) %o% colSums(lambd_ng_endo) / sum(lambd_ng_endo),
        2,
        rho_n / (1 - rho_n),
        '*'
    )
    ...
    counts_cell <- matrix(
        rpois(
            length(lambd_ng_cell),
            as.vector(lambd_ng_cell)
        ),
...
simulate_guidebender2 <- function(
    ...
    d_mu_cell_drop_diff <- (log(frac_signal / frac_endo / avg_infections) -
        .5 * d_sigma_cell^2 -
        .5 * d_sigma_guide^2 +
        .5 * d_sigma_drop^2)
